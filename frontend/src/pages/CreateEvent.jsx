...
const CreateEvent = () => {
  ...
  useEffect(() => {
    // Show message if template is selected
    if (templateId) {
      toast.info('Şablon seçildi! İndi tədbir məlumatlarını daxil edin.');
    }
  }, [templateId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If template was selected, redirect to template editor
      // Otherwise, redirect to event detail page
      if (templateId) {
        navigate(`/editor/${response.data.id}?template=${templateId}`);
      } else {
